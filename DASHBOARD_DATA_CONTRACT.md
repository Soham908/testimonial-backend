...
**Also 2026-09-21**: two more filter additions to `GET /dashboard/
...
see the route's row below) and `life_skill` (segments whose
`extracted.life_skills_mentioned` contains the given value, same
`jsonb_array_elements_text` pattern as `theme`). Plus a real bug fix, found
while confirming `GET /dashboard/life-skills` against production data: on
`GET /dashboard/response/:segment_id`, `sentiment_score`/`highlight_score`
were serializing as strings (`"0.6"`) instead of numbers, because that
route is a plain Prisma model query rather than `$queryRaw` with an
explicit `::float` cast like every other endpoint here — Prisma's `Decimal`
type serializes via `.toJSON()` as a string. Fixed with `.toNumber()` at
the source; confirmed against real Neon data before and after.
