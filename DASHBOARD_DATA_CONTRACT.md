...
| `extracted` | `Json` (fixed shape as of 2026-09-16 — see §2) | yes | `/dashboard/teacher-impact` (`extracted->>'mentions_teacher'`, `extracted->>'teacher_contribution'`); `/dashboard/life-skills` (added 2026-09-21, `jsonb_array_elements_text(extracted->'life_skills_mentioned')`) |
...
**Updated 2026-09-21**: three more, found while planning the dashboard's
Education Experience page. First, confirmed `GET /dashboard/wordcloud`
already supported an omitted `question_index` (pools every question
network-wide) — no code change needed there, just a check. Second,
`video_url` was added to each item in `GET /dashboard/highlights`' array
(same signed-URL generation as `GET /dashboard/response/:segment_id`, so a
grid of several video thumbnails doesn't need one request per item) — never
`null` here, unlike the single-lookup endpoint, since the query already
excludes every `moderation_flag: true` row before any URL is generated.
Third, a new endpoint, `GET /dashboard/life-skills` — same shape/denominator
convention as `GET /dashboard/teacher-impact`, counting segments per skill
in `extracted.life_skills_mentioned`. See all three rows below.

**Also 2026-09-21**: three more filter additions to `GET /dashboard/
highlights`, same independently-optional/intersecting pattern as
`question_index`/`theme` — `teacher_contribution` (requires
`extracted.mentions_teacher: true` and a matching
`extracted.teacher_contribution`; no separate "not specified" value exists,
see the route's row below), `life_skill` (segments whose
`extracted.life_skills_mentioned` contains the given value, same
`jsonb_array_elements_text` pattern as `theme`), and `sentiment_category`
(`positive`/`neutral`/`negative`, reusing the exact `POSITIVE_THRESHOLD`/
`NEGATIVE_THRESHOLD` constants `GET /dashboard/summary`'s `sentiment_split`
already computes its percentages from, so the summary donut and this
filter's drawer can never silently disagree on the boundary — verified the
three buckets are mutually exclusive and exhaustive against real Neon data,
67 + 79 + 5 = 151 matching the total unflagged row count exactly). Plus a
real bug fix, found while confirming `GET /dashboard/life-skills` against
production data: on `GET /dashboard/response/:segment_id`,
`sentiment_score`/`highlight_score` were serializing as strings (`"0.6"`)
instead of numbers, because that route is a plain Prisma model query rather
than `$queryRaw` with an explicit `::float` cast like every other endpoint
here — Prisma's `Decimal` type serializes via `.toJSON()` as a string.
Fixed with `.toNumber()` at the source; confirmed against real Neon data
before and after.
...
**Character**: all 11 routes are read-only, no LLM call. 9 of them are
SQL-aggregated (`prisma.$queryRaw` tagged templates, not the query builder —
needed for `jsonb_array_elements_text` theme/life-skill-unnesting and
`FILTER (WHERE ...)` bucket counts); `GET /dashboard/response/:segment_id`
and `GET /dashboard/questions` are the two exceptions — plain Prisma query
builder lookups, not raw SQL, since neither is an aggregate. Every count-based stat
returns the raw count alongside any percentage — small buckets aren't hidden.

| Route | Returns |
|---|---|
| `GET /dashboard/wordcloud?question_index=N&limit=100` | `{ question_index: number\|null, transcript_count, words: [{word, count}] }` — code-only tokenization (no LLM) across `Transcript.text` for the client. **`question_index` is, and always was, optional** (confirmed 2026-09-21, not a new change) — omit it to pool every question network-wide, same `undefined`/`null` param-parsing pattern `/dashboard/highlights` now also uses. `src/services/wordFrequency.ts`: Unicode letter-run matching (handles Devanagari as well as Latin), lowercased, English **and** Hindi **and** Marathi stopwords all applied to every transcript regardless of detected language — transcripts here are routinely code-mixed, so a single-language list would under-filter the other two. |
...
| `GET /dashboard/highlights?question_index=N&theme=<theme_name>&teacher_contribution=<value>&life_skill=<value>&sentiment_category=<value>&limit=10` | `{ question_index: number\|null, theme: string\|null, teacher_contribution: string\|null, life_skill: string\|null, sentiment_category: string\|null, highlights: [{segment_id, best_quote, highlight_score, sentiment_score, language, distributor_name, actionable_feedback, video_url}] }`, ordered by `highlight_score` desc. **All five filters are independently optional and intersect when combined (question_index/theme changed 2026-09-19; teacher_contribution/life_skill/sentiment_category added 2026-09-21)**: omit all for every highlight for the client, any subset to filter by just those, several together for their intersection. All five echo back `null` in the response when omitted. **Excludes `moderation_flag: true` rows** — the first real consumer of the corrected (§2) semantics. Pre-2026-09-16 rows are still under the old inverted meaning, so until reprocessed this filter under-includes old content (never over-includes/leaks anything), the safe failure direction. **`distributor_name` added 2026-09-17 — a deliberate, endpoint-specific exception to this router's "no identity" design** (internal-only viewing, two known people, most participants already personally known to them); does not extend to any other endpoint or a future client-facing version without a fresh decision. `actionable_feedback` (also added 2026-09-17) is `null` unless `contains_complaint` was true for that segment. **`theme` (added 2026-09-18)** — must be one of `THEME_VALUES` (§2) or `400`; filters to segments whose `themes` array contains it, checked via `jsonb_array_elements_text`, not the `?` jsonb operator. **`video_url` (added 2026-09-21)** — per-item signed S3 GET URL, same `getPlaybackUrl` helper as `/dashboard/response/:segment_id` below, so a grid of thumbnails needs one request, not one per item; never `null` here (unlike that endpoint) since the query's own `moderation_flag = false` filter already guarantees every row reaching this point is unflagged. **`teacher_contribution` (added 2026-09-21)** — must be one of `TEACHER_CONTRIBUTION_VALUES` (§2) or `400`; also implicitly requires `extracted.mentions_teacher: true` (per the schema, `teacher_contribution` is only ever `null` when that's `false` — no separate "not specified" value exists to filter on). **`life_skill` (added 2026-09-21)** — must be one of `LIFE_SKILL_VALUES` (§2) or `400`; filters to segments whose `extracted.life_skills_mentioned` array contains it, same `jsonb_array_elements_text` pattern as `theme` and `GET /dashboard/life-skills` below. **`sentiment_category` (added 2026-09-21)** — one of `positive`/`neutral`/`negative` (`400` otherwise), reusing the exact `POSITIVE_THRESHOLD`/`NEGATIVE_THRESHOLD` constants `GET /dashboard/summary`'s `sentiment_split` computes from — same boundary, defined once, never re-derived; the three buckets are mutually exclusive and exhaustive over unflagged, analyzed segments. |
| `GET /dashboard/response/:segment_id` | **Added 2026-09-17.** Single-segment lookup, not a list — `{ segment_id, question_index, video_url: string\|null, transcript: {text, language_detected, language_probability}|null, sentiment: {sentiment_score, themes, emotional_tone, summary, best_quote, is_relevant, moderation_flag, contains_profanity, contains_complaint, actionable_feedback, highlight_score, extracted}|null }`. `segment_id` must be a well-formed UUID (`400` otherwise); scoped by `client_id` like every other route here — a segment belonging to a different client is indistinguishable from a nonexistent one (`404` either way). `transcript`/`sentiment` are `null`, not a 404, when that stage just hasn't completed yet. No distributor identity field — unlike `/dashboard/highlights` above, not asked for here. **`video_url` (added 2026-09-18)** — signed S3 GET URL for `Segment.video_key` via the same `getPlaybackUrl` helper mobile playback uses (1hr expiry, generated fresh every request, nothing new stored); `null` when `moderation_flag` is `true`, same internal-only "not individually surfaced" reasoning as `distributor_name` above. **Fixed 2026-09-21**: `sentiment.sentiment_score`/`sentiment.highlight_score` now correctly serialize as JSON numbers — this route is a plain Prisma model query (not `$queryRaw` with a `::float` cast like every other route here), so Prisma's `Decimal` type was silently serializing them as strings (`"0.6"`) via `Decimal.toJSON()`; fixed with `.toNumber()` at the source, confirmed against real Neon data before/after. |
...
| `GET /dashboard/life-skills` | **Added 2026-09-21.** `{ total_analyzed, life_skills: [{life_skill, count, percentage}] }`, ordered by `count` desc — counts segments per skill in `extracted.life_skills_mentioned`, across the 5 fixed `LIFE_SKILL_VALUES` (§2: `communication`, `financial_literacy`, `leadership`, `teamwork`, `problem_solving`), via `jsonb_array_elements_text(sr."extracted"->'life_skills_mentioned')`. Same shape/denominator convention as `GET /dashboard/teacher-impact` just above; its own endpoint for the same "own analytical dimension, not a top-line number" reason teacher-impact isn't folded into `/dashboard/summary` either. Unlike `teacher_contribution`, there's no gating boolean — a segment can mention 0–5 skills, so `count`s don't sum to `total_analyzed` and each `percentage` is independently of `total_analyzed`. Only populated for segments analyzed after 2026-09-16 (same as `teacher-impact`'s `extracted` dependency); `extracted` being nullable is handled safely — `jsonb_array_elements_text` on a SQL `NULL` input yields zero rows, not an error (confirmed live against the dev DB), so pre-rewrite rows simply contribute nothing rather than breaking the query. |
